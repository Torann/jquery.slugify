...
;(function ($) {
    ...
    $.fn.slugify = function (source, options) {
        ...
        var settings = $.extend({
            slugFunc: (function (val, originalFunc) {
                return originalFunc(val);
            })
        }, options);
        ...
        var updateSlug = function () {
            ...
            if (targetIsInput) {
                $target.filter(':not(.slugify-locked)').val(slug).trigger('keyup');
            } else {
    ...
    $(window).on('load', function () {
        $('[data-slugify]').each(function () {
            var $input = $(this),
                target = $input.data('slugify');

            $(target).slugify($input);
        })
    })
