...
;(function ($) {
    ...
    var convertToSlug = function (value) {
        ...
        for (var i = 0; i < value.length; i++) {
            str += map[value.charCodeAt(i).toString(16)] || "";
        }
    ...
    var Plugin = function (element) {
        // Prevent duplicates
        if (element.getAttribute('data-slug-ready')) return;

        var $element = $(element),
            $target = $($element.data('slugify')),
            targetIsInput = $target.is('input') || $target.is('textarea');

        element.setAttribute('data-slug-ready', true);

        $element.on('keyup.slugify change.slugify paste.slugify', function (ev) {
            var slug = convertToSlug($(this).val());

            if (slug) {
                if (targetIsInput) {
                    $target.val(slug).trigger('keyup');
                }
                else {
                    $target.text(slug);
                }
        ...
        var onChange = function () {
            ...
            $target.on('change.slugify', onChange);
        };
    ...
    $(document).on('touchstart click', '[data-slugify]', function (ev) {
        if (!ev.target.getAttribute('data-slug-ready')) {
            Plugin(ev.target);
        }
    });
